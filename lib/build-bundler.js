...
module.exports = opts => {
	...
	return (file, lookupAsset) => {
		...
		if(previouslyIncluded(file, previouslyIncludedFiles)) {
...
function buildFileWriter(opts) {
	let { fingerprint, targetDir, outputFilename, configDir, inputFileName } =
		opts;
	...
	return input => {
		return new Promise((resolve, reject) => {
			...
			fs.writeFile(outputFilename, input.css, err => {
				if(err) {
				...
				resolve({
					...
					output: path.relative(targetDir, outputFilename),
...
function previouslyIncluded(file, previouslyIncludedFiles) {
	return previouslyIncludedFiles === undefined ||
		previouslyIncludedFiles.find(candidate => candidate === file);
}
