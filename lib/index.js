...
function faucetSass(config, assetManager, { compact, sourcemaps } = {}) {
...
function makeBundler(config, assetManager, { compact, sourcemaps } = {}) {
	...
	let browsers = determineBrowsers(browserslist, assetManager.referenceDir);
...
function determineBrowsers(browserslistEnv, referenceDir) {
	if(browserslistEnv === false) {
		return null;
	}
	...
	if(browserslistEnv) {
		if(!browsers[browserslistEnv]) {
			abort(`ERROR: The browserslist environment ${repr(browserslistEnv)} you requested is not defined`);
		}
		return browsers[browserslistEnv];
	}

	return browsers.defaults;
}

function previouslyIncluded(filepaths, previouslyIncludedFiles) {
	return previouslyIncludedFiles === undefined ||
		filepaths.some(filepath => previouslyIncludedFiles.
			find(candidate => candidate === filepath));
}
