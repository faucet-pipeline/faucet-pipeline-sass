...
let knownConfigParameters = ["source", "target", "fingerprint", "browserslist"];
...
function makeBundler(config, assetManager, { browsers, compact, sourcemaps } = {}) {
	...
	let unknownConfigParameters = determineUnknownConfigParameters(Object.keys(config));
	if(unknownConfigParameters.length > 0) {
		abort(`ERROR: Unknown configuration parameters:
		${unknownConfigParameters.join(", ")}`);
	}
...
function determineUnknownConfigParameters(providedKeys) {
	return providedKeys.filter(x => !knownConfigParameters.includes(x));
}
